...
%     load('mints_node_3_2_Air_Mar_data_from_30-Apr-2019_to_30-Aug-2019_in_30_sec_averaged_slices_for_Node_001e06305a12.mat')
    eval(strcat("load('",deliverablesFolder,"/mints_node_3_2_Air_Mar_data_from_",...
       string(startDate),"_to_",string(endDate),"_in_",...
       strrep(string(dt)," ","_"),"_averaged_slices_for_Node_",nodeID,"')"));
    
    modelsFolder = modelsFolderPre +"/"+strrep(string(dt)," ","")+"/"
   mints= rmmissing(mintsRetimed);
    %--------------------------------------------------------------------------
    ...
    %--------------------------------------------------------------------------
    % Training Version
    VersionSt = ['Version_RE_AirMar' datestr(today,'yyyy_mm_dd')];
    disp(VersionSt)
